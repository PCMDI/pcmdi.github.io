...
if ($button == 'Cancel')
{
    header('Location: project_detail.php?ipcc_subproject_id=' . $ipcc_subproject_id);
    exit();
}
else if ($button == 'Save')
{
    ...
    if (count($errors) == 0)
    {
        $sql = "UPDATE pcmdi_web.ipcc_publications SET ";
        $sql .= "authors='" . $authors . "', ";
        $sql .= "title='" . $pub_title . "', ";
        ...
        $sql .= "WHERE ipcc_publication_id='" . $ipcc_publication_id . "'";
...
else
{
    ...
    $sql .= "WHERE ipcc_publication_id='" . mysql_real_escape_string($ipcc_publication_id) . "'";
...
<form name="publication_form" id="publication_form" action="edit_publication.php" method="post">
...
<input type="hidden" id="ipcc_publication_id" name="ipcc_publication_id" value="<?php print $ipcc_publication_id; ?>">
...
<table cellpadding="0" cellspacing="2" border="0">
    ...
    <tr>
        <td>&nbsp;</td>
        <td>(e.g. "Johnson, L.E., and J.M. Smith")</td>
    </tr>
