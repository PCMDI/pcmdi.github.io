...
<table width="100%">
    <tr>
        <td valign="top">
            <a href="diagnostic_subprojects.php">WCRP CMIP3 Subprojects</a> | <b>Subproject Publications</b>
        </td>
                        ...
                        if ($pub['journal'] != '')
                        {
                            ...
                            if ($pub['volume'] != '' || $pub['pages'] != '' || $pub['doi_id'] != '')
                            {
                                ...
                                if ($pub['doi_id'] != '')
                                {
                                    ...
                                    if (stripos($pub['doi_id'], 'isbn') === false &&
                                        stripos($pub['doi_id'], 'sref') === false)
                                    {
                                        $doi_id = trim($pub['doi_id']);
                                        $doi_id = str_replace('DOI ', 'doi:', $doi_id);
                                        $doi_id = str_replace('DOI: ' , 'doi:', $doi_id);
                                        $doi_id = str_replace('doi: ', 'doi:', $doi_id); 
                                        $citation .= '<a href="http://dx.doi.org/' . $doi_id . '">' . $pub['doi_id'] . '</a>';
                                    }
                                    else
                                    {
                                        $citation .= $pub['doi_id'];
